...
class Order(models.Model):
    ...
    # ✅ الجديد: طلب معاينة قبل الاستلام
    wants_inspection = models.BooleanField(
        default=False,
        verbose_name="طلب معاينة قبل الاستلام"
    )
    inspection_fee = models.DecimalField(
        max_digits=10, decimal_places=2, default=0.00,
        verbose_name="رسوم المعاينة"
    )
    ...
    city = models.CharField(max_length=100, default="Cairo", db_index=True)
    payment_screenshot = models.ImageField(upload_to="payments/instapay/%Y/%m/", blank=True, null=True)
    ...
    # ✅ سعر المنتجات بدون الشحن والمعاينة
    @property
    def subtotal(self):
        return self.total_amount - self.shipping_price - self.inspection_fee
