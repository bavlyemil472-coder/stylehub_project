...
# استيراد دوال الدفع الخاصة بـ Paymob
...
class CreateOrderView(APIView):
    ...
    def post(self, request):
        ...
        # 2. استلام بيانات الشحن وطريقة الدفع من الطلب (Request Data)
        ...
        total = 0
        # 4. نقل المنتجات من السلة إلى تفاصيل الطلب (OrderItem)
        for item in cart.items.all():
            variant = item.variant
            
            # التأكد من توفر المخزون
            if item.quantity > variant.stock:
                # تراجع عن العملية كلها إذا وجد نقص في المخزون
        ...
        # 5. تحديث إجمالي الطلب ومسح السلة
...
class StartPaymentView(APIView):
    ...
    def post(self, request, order_id):
        ...
        # تحويل المبلغ لـ Cents (قرش) كما تطلب Paymob
...
def paymob_webhook(request):
    ...
    if data.get("success") == "true":
        # معرف الطلب الذي أرسلناه لـ Paymob
        ...
        if order:
            ...
            order.status = 'processing'  # تحويل حالة الطلب لبدء التجهيز
            order.save()
...
class AdminOrderListView(APIView): # استخدام APIView مباشرة أضمن للـ Debugging
    ...
    def get(self, request):
        try:
            ...
            print(f"Error in AdminOrderList GET: {e}") # هيظهرلك في الترمينال
            return Response({"error": str(e)}, status=500)

    def patch(self, request, pk): # الـ pk هييجي من الـ URL مباشرة
        try:
