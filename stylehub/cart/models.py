...
def auto_restore_stock(sender, instance, **kwargs):
    ...
    # ✅ الحماية الجديدة: لو الـ variant نفسه اتمسح (مثلاً بسبب حذف المنتج كله)
    # مفيش داعي نرجع مخزون لحاجة مش موجودة أصلاً، فبنتجاهل بهدوء بدل ما نوقع السيرفر
    if not instance.variant_id:
        return

    try:
        variant = ProductVariant.objects.get(pk=instance.variant_id)
