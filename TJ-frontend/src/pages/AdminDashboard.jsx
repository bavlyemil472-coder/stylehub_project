...
import { DollarSign, ShoppingBag, Clock, Activity, TrendingUp, ChevronLeft, RefreshCw } from 'lucide-react';
                                ...
                                <tbody className="divide-y divide-white/5">
                                    {orders.map((order) => (
                                        <tr key={order.id} className="group hover:bg-white/[0.02] transition-colors">
                                            ...
                                            <td className="py-5 text-sm font-bold uppercase">{order.full_name || 'عميل مجهول'}</td>
                                            <td className="py-5 text-sm text-brand-gold font-bold">{order.total_amount} EGP</td>
                                            <td className="py-5">
                                                <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase ${
                                                    order.status === 'delivered' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-brand-gold/10 text-brand-gold'
                                                }`}>
                                                    {order.status}
                                                </span>
                                            </td>
                                            <td className="py-5">
                                                <select 
                                                    className="bg-brand-dark border border-white/10 rounded-lg text-[10px] p-2 focus:outline-none focus:border-brand-gold"
                                                    value={order.status}
